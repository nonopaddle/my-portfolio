...
export const Navbar = ({ section_list, selected, select }: HeaderType) => {
	...
	return (
		<>
			<nav className={'h-full w-fit absolute transition-all duration-300 pl-5 pr-5 pt-20 flex-none flex flex-col z-20 bg-blue-primary bg-opacity-90 backdrop-blur ' + (collapse && '-translate-x-1/2 opacity-0 invisible')}>
				...
				<ul className={"flex flex-col justify-center space-y-3 relative"}>
						{section_list?.map((section: any, index: number) => (
							<li key={section.label} className="group px-3 z-10">
								...
								group-hover:after:w-full group-hover:after:left-0 ` + (`${index}` === selected ? 'after:w-full after:left-0 ' : 'after:w-0 after:left-1/2 ') + (collapse ? '-left-10' : 'left-0')}>
									{section.label}
								</button>
							</li>
						))}
